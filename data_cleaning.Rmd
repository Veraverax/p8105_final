...
states_merged <- geo_join(states, wine_us, "STUSPS", "state")

states_merged <- subset(states_merged, !is.na(total))

pal <- colorNumeric("Blues", domain = states_merged$total)

popup_wine <- paste0("Wineries: ", as.character(states_merged$total))

leaflet() %>%
  addProviderTiles("CartoDB.Positron") %>%
  setView(-98.483330, 38.712046, zoom = 4) %>% 
  addPolygons(data = states_merged, 
              fillColor = ~pal(states_merged$total), 
              fillOpacity = 0.7, 
              weight = 0.2, 
              smoothFactor = 0.2, 
              popup = ~popup_wine)
