...
wine_top10_word <- function(wine_subset){
  ...
  other_meaningless <- data.frame(word = c("style", "character", "vineyard", "wine","drink","flavors","notes","note", "finish", "structure", "blend", "touch", "feels", "nose", "age",  "palate", "cabernet", "pinot","noir", "chardonnay", "sauvignon", "aromas", "black", "red", "white", "rose", "rosé", "sparkling"))
  wine_word = 
    wine_word %>% 
...
## New-world red wine
```{r message = FALSE}
wine_word_cloud(new_red, "red", "white")
```

## Old-world red wine
```{r message = FALSE}
wine_word_cloud(old_red, "red", "white")
```

# White wine
```{r message = FALSE}
wine_word_cloud(white_df, "red", "white")
```

## New-world white wine
```{r message = FALSE}
wine_word_cloud(new_white, "red", "white")
```

## Old-world white wine
```{r message = FALSE}
wine_word_cloud(old_white, "red", "white")
```

# Rose
```{r message = FALSE}
wine_word_cloud(rose_df, "red", "white")
```

# Sparkling
```{r message = FALSE}
wine_word_cloud(sparkling_df, "red", "white")
```
...
library(webshot)
library("htmlwidgets")
red_word <- wine_word_cloud(red_df, "red", "white")
saveWidget(red_word, "tmp.html", selfcontained = F)
webshot("tmp.html", "./image/red_word.png", delay = 5, vwidth = 500, vheight = 500)
