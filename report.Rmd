...
* A [project](https://web.stanford.edu/~kjytay/courses/stats32-aut2019/projects/wine_report.html) that analyzed the same dataset to evaluate what characteristics of a wine affect its perceived quality, and what characteristics are correlated with each other
...
## Word  Cloud
First, a tidy dataset was created with each word on a separate row by un-nesting the tokens in each row. Then, "stop word", numbers and other uninformative words were removed, so that we could focus on tasting notes. Finally, the wordcloud2 function was applied to generate the word clouds, which would present the key tasting notes by showing frequent words in larger font sizes.
...
Some key tasting notes for wines are fruit, cherry, tannins, and acidity. Overall, higher-rated wines share many key words with lower-rated wines. Regression analysis would take a deeper look by comparing these differences among specific wine type groups.
...
* The premium segment is, on average 3.07 pts lower than the luxury segment; the value segment is 6.27 points lower than the luxury on average.
...
* The old world wines are, on average 0.19 pts lower than the unidentified category, while the new world wines are 0.43 pts lower than the unidentified category. This was unexpected, but still reasonable as red wines are the wine type where the country of origin is more heavily discussed and compared, while discussion of white wine is less focused on the old vs new world comparison.
...
* **Year**: Although there is a general perception that older wines are better, that does not always hold true. We omitted the year as a predictor in the final multivariate model for two reasons. First, a linear relationship using the year as a continuous variable is not appropriate to describe the association between wine rating and year. Besides, the dataset does not provide information on when the wine rating is generated, which is susceptible to bias when assessing the association.

* **Word Descriptions**: Due to the limited information on tasting notes for a bottle of wine in our dataset, we could only simplify the word identification process and chose 3-5 keywords as used in the Vivino app. To better understand wine ratings and incorporate more information about tasting notes, more reviews are needed. This model provides only a basic sense of whether certain notes correspond to higher ratings.

* **Old vs New World**: Since there is not a comprehensive list of countries to categorize all wines, we relied on the best possible list by Wine Folly. However, there could be wines that belong to either new or old world that are currently categorized as other (reference group).

* **Price**: In the multivariate model, we used price segmentation rather than price as a continuous variable to fit the model. Categorization price is the appropriate method to better describe the rating outcome, but only segmenting into three categories could be over-simplifying the true and complicated real world business indications of wine pricing.
